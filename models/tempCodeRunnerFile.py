...
class traffic_env:
    def __init__ (self, network_file, congested = [], traffic_light = [], evaluation = "", congestion_level = "", travel_speed = 80):
        ...
        # Store initial congestion/TL parameters as base or default values
        self.base_congested_input = congested
        self.base_congestion_level = congestion_level.lower()
        self.initial_num_edges = len(self.edges)
        
        # Initialize congestion based on input (will be updated dynamically later)
        if self.base_congested_input:
            self.congested_edges = [item[0] for item in self.base_congested_input]
            self.congestion_duration = [item[1] for item in self.base_congested_input]
            ...
            # Determine initial congestion level based on string input
            if self.base_congestion_level == "low":
                traffic_level_pct = 0.05
            elif self.base_congestion_level == "medium":
                traffic_level_pct = 0.10
            elif self.base_congestion_level == "high":
                traffic_level_pct = 0.20
            else:
                # Default to medium if no specific level is provided
                traffic_level_pct = 0.10 
            
            # Initial random congestion assignment
            num_congested = round(self.initial_num_edges * traffic_level_pct)
            self.congested_edges = random.sample(self.edges, num_congested)
            self.congestion_duration = [random.randint(10, 20) for _ in range(num_congested)]
        ...
        # Define traffic lights nodes (usually static)
    ...
    # --- NEW METHOD FOR EPISODIC DYNAMICS ---
    def update_dynamic_costs(self, traffic_level_percentage=0.15, min_penalty=10, max_penalty=30):
        """
        Randomly updates the set of congested edges and their penalties 
        to simulate a change in traffic conditions for a new episode.
        
        Args:
            traffic_level_percentage (float): Percentage of edges to be congested.
            min_penalty (int): Minimum penalty time (in minutes).
            max_penalty (int): Maximum penalty time (in minutes).
        """
        
        # 1. Determine the number of congested edges 
        num_congested = round(self.initial_num_edges * traffic_level_percentage)
        
        # 2. Randomly select the edges to be congested
        self.congested_edges = random.sample(self.edges, num_congested)
        
        # 3. Assign new random penalty durations for these edges
        self.congestion_duration = [random.randint(min_penalty, max_penalty) for _ in range(num_congested)]
        
        # Note: Traffic Light penalties (self.tl_nodes and self.tl_duration) remain static.
